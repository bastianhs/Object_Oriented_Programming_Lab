...
public class Pendekar implements Kelas {
    ...
    public void serang(Karakter self, Karakter target) {
        System.out.format("%s, sang pendekar, menyerang %s dengan %s%n", self.getName(), target.getName(), getSenjata());
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if (target.isHidup()) {
            System.out.format("%s memukul mundur %s dengan %s%n", self.getName(), target.getName(), getPerisai());
            target.terimaSerangan(40);
        } else {
            System.out.format("%s sudah mati, tidak bisa dipukul mundur%n", target.getName());
        }
    }
    ...
    public void gunakanUltimate(Karakter self, Karakter target) {
        if (target.isHidup()) {
            System.out.format("%s menyerukan nada peperangan%n", self.getName());
            self.addHealth(30);
            self.addMana(20);
            System.out.format("%s mendapatkan 30 health dan 20 mana%n", self.getName());
            System.out.format("%s menyerang %s dengan %s dan %s%n", self.getName(), target.getName(), getSenjata(), getPerisai());
